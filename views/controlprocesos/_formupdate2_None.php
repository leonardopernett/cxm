...
$this->title = 'Actualizar la Valoraciones';
    ...
    $txtCantidadcxm = Yii::$app->db->createCommand("select sum(cant_valor) from tbl_control_params where anulado = 0 and evaluados_id = $txtIdusua and fechacreacion between '$txtfechainicio' and '$txtfechafin'")->queryScalar();

    $varTotalspeech = Yii::$app->db->createCommand("select sum(cant_valor) from tbl_control_focalizada where anulado = 0 and fechacreacion between '$txtfechainicio' and '$txtfechafin' and evaluados_id = $txtIdusua")->queryScalar();

    if ($txtCantidadcxm == null) {
        $txtCantidadcxm = 0;
    }else{
        if ($varTotalspeech == null) {
            $varTotalspeech = 0;
        }
    }

    $txtCantidad = $txtCantidadcxm  + $varTotalspeech;
                ...
                <?= GridView::widget([
                        ...
                        'columns' => [
                            [
                                ...
                                'value' => function($data){
                                        return $data->getspeech1($data->idcontrolfocalizada);
                                }
                            ],
                            [
                                'attribute' => 'Dimensiones',
                            ...
                            [
                                ...
                                [
                                    'update' => function ($url, $model) {
                                        return Html::a('<span class="glyphicon glyphicon-pencil"></span>', ['updatetwospeech', 'id' => $model->idcontrolfocalizada, 'varfecha' => $model->fechacreacion, 'evaluadoId' => $model->evaluados_id]);
                                    },
                                    'delete' => function($url, $model){
                                        return Html::a('<span class="glyphicon glyphicon-trash"></span>', ['deletetwospeech', 'id' => $model->idcontrolfocalizada, 'varfecha' => $model->fechacreacion, 'evaluadoId' => $model->evaluados_id], [
                                            'class' => '',
                                            'data' => [
                                                'method' => 'post',
                                            ],
