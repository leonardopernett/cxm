...
$this->title = 'Agregar Valorador';
    ...
    $varTotalcxm = Yii::$app->db->createCommand("select sum(cant_valor) from tbl_control_params where anulado = 0 and fechacreacion > '$Mesactual' and evaluados_id = $txtusua_id")->queryScalar();

    $varTotalspeech = Yii::$app->db->createCommand("select sum(cant_valor) from tbl_control_focalizada where anulado = 0 and fechacreacion > '$Mesactual' and evaluados_id = $txtusua_id")->queryScalar();

    if ($varTotalspeech == null) {
        $varTotalspeech = 0;
    }

    $varTotal = $varTotalcxm + $varTotalspeech;
?>
<style type="text/css">
                ...
                <?= GridView::widget([
                        ...
                        'columns' => [
                            ...
                            [
                                ...
                                [
                                    'update' => function ($url, $model) {
                                        return Html::a('<span class="glyphicon glyphicon-pencil"></span>', ['updatespeech', 'id' => $model->idcontrolfocalizada, 'evaluadoId' => $model->evaluados_id]);
                                    },
                                    'delete' => function($url, $model){
                                        return Html::a('<span class="glyphicon glyphicon-trash"></span>', ['deletespeech', 'id' => $model->idcontrolfocalizada, 'evaluadoId' => $model->evaluados_id], [
                                            'class' => '',
                                            'data' => [
                                                'method' => 'post',
                                            ],
                ...
                if (varTipoCort2 == "") {
                    ...
                    $.ajax({
                        ...
                        success : function(response){ 
                            ...
                            // window.open('http://127.0.0.1/qa_pruebas/web/index.php/controlprocesos/index','_self');
