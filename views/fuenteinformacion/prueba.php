...
class Ejecucionformularios extends \yii\db\ActiveRecord {
    ...
    public function attributeLabels() {
        return [
            ...
            'i3_nmcalculo' => Yii::t('app', 'CARIÃƒâ€˜O/'),
    ...
    public function getReportFormularios($bandera) {
        ...
        $sql = "SELECT DISTINCT t.id, t.created,e.name evaluado, 
                ...
                rc.director_programa as director,
    ...
    public function extractConsTransORIGINAL() {
        ...
        //SI SOLO HABIA UNA VALORACIÃƒâ€œN PINTO LOS TITULOS
        if ($printTitle) {
            foreach ($titulos as $value) {
                $filecontent .= utf8_decode($value['header']) . "|";
            }
    ...
    public function extractConsTrans() {
        ...
        if($this->arbol_id == '3104'){
            ...
            $titulos[20] = ['header' => 'Identificacion', 'value' => '20'];
            $titulos[21] = ['header' => 'Nombre', 'value' => '21'];
            ...
            if (count($data) > 0) {
                ...
                foreach ($data as $i => $row) {
                    ...
                    if ($row['fid'] != $fid) {
                        ...
                        if($this->arbol_id == '3104'){
                            $dataProvider[$newRow][19] = $this->vData($data[$i]['identificacion']);
                            $dataProvider[$newRow][20] = $this->vData($data[$i]['nombre']);
                            $dataProvider[$newRow][21] = $this->vData($data[$i]['ani']);
                            $dataProvider[$newRow][22] = $this->vData($data[$i]['rn']);
                            $dataProvider[$newRow][23] = $this->vData($data[$i]['tipologia']);
                            $dataProvider[$newRow][24] = $this->vData($data[$i]['pregunta1']);
                            $dataProvider[$newRow][25] = $this->vData($data[$i]['reason3']);
                            $dataProvider[$newRow][26] = $this->vData($data[$i]['reason4']);
                            $dataProvider[$newRow][27] = $this->vData($data[$i]['idnps']);
                            $dataProvider[$newRow][28] = $this->vData($data[$i]['email']);
                            $dataProvider[$newRow][29] = $this->vData($data[$i]['nmusuario']);
                            $dataProvider[$newRow][30] = $this->vData($data[$i]['user_seg']);
                            $dataProvider[$newRow][31] = $this->vData($data[$i]['nota_anterior']);

                            if ($data[$i]['ejec_principal'] != '' && $data[$i]['estado'] != '') {
                                $dataProvider[$newRow][32] ='ValoraciÃ³n Escalada';
                            } else {
                                if ($data[$i]['ejec_principal'] != '') {
                                    $dataProvider[$newRow][32] ='ValoraciÃ³n Adicional, Id valoraciÃ³n principal:' . $data[$i]['ejec_principal'];
                                }else{
                                    $dataProvider[$newRow][32] = 'N/A';
                                }
                            }
                            $dataProvider[$newRow][33] = $this->vData($data[$i]['score']);
                            $dataProvider[$newRow][34] = $this->vData($data[$i]['fi1_nmcalculo']);
                            $dataProvider[$newRow][35] = $this->vData($data[$i]['fi2_nmcalculo']);
                            $dataProvider[$newRow][36] = $this->vData($data[$i]['fi3_nmcalculo']);
                            $dataProvider[$newRow][37] = $this->vData($data[$i]['fi4_nmcalculo']);
                            $dataProvider[$newRow][38] = $this->vData($data[$i]['fi5_nmcalculo']);
                            $dataProvider[$newRow][39] = $this->vData($data[$i]['fi6_nmcalculo']);
                            $dataProvider[$newRow][40] = $this->vData($data[$i]['fi7_nmcalculo']);
                            $dataProvider[$newRow][41] = $this->vData($data[$i]['fi8_nmcalculo']);
                            $dataProvider[$newRow][42] = $this->vData($data[$i]['fi9_nmcalculo']);
                            $dataProvider[$newRow][43] = $this->vData($data[$i]['fi10_nmcalculo']);
                        } else{
    ...
    /**
     * Si viene algÃƒÂºn dato retorna el dato, de lo contrario retorne " - "
     * 
     * @return string
     */
    ...
    public static function getDatabygraf($arbol_id, $dimension_id, $fecha, $metrica, $banderaGrafica, $model, $control, $volumenes = false, $segundoCalifPer = false) {
        ...
        switch ($metrica) {
            case 13:
                ...
                break;
            case 14:
                $sql->andWhere("satu.tipologia = 'FELICITACIï¿½N CON BUZï¿½N'");
                break;
            case 15:
                $sql->andWhere("satu.tipologia = 'CRITICA'");
                break;
            case 16:
                $sql->andWhere("satu.tipologia = 'CRITICA POR BUZï¿½N'");
                break;
            case 17:
    ...
    public static function getDatabytabletotalexcel($dimension_id, $fecha, $metrica, $arbol, $model, $volumenes = false, $idMetrica = null) {
        ...
        switch ($metrica) {
            ...
            case 15:
                ...
                break;
            case 16:
                $sql->andWhere("AND satu.tipologia = 'CRITICA POR BUZï¿½N'");
                break;
            case 17:
