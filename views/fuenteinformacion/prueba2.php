...
class Ejecucionformularios extends \yii\db\ActiveRecord {
    ...
    public function attributeLabels() {
        return [
            ...
            'created' => Yii::t('app', 'Fecha de Valoracion'),
            ...
            'usua_id_modifica' => Yii::t('app', 'Id Usuario/Modificacion'),
            'modified' => Yii::t('app', 'Fecha de Modificacion'),
            ...
            'i3_nmcalculo' => Yii::t('app', 'CARIÃ‘O/'),
    ...
    public function getReportFormularios($bandera) {
        ...
        $sql = "SELECT DISTINCT t.id, t.created,e.name evaluado, 
                ...
                rc.cliente AS codcliente, rc.pcrc AS centrocosto, rc.ciudad as ciudad, 
                rc.director_programa as director, rc.gerente as gerente,
                ...
                t.evaluado_id, t.formulario_id, t.id fid ,t.usua_id ideva, t.ejec_principal, t.escalado
		...
		LEFT JOIN tbl_registro_ejec_cliente rc ON rc.ejec_form_id = t.id
		...
		INNER JOIN tbl_tmpreportes_arbol tra ON tra.arbol_id = t.arbol_id
                ...
                WHERE t.created >= ' " . $startDate . "' 
                AND t.created <= '" . $endDate . "'" . $and . " 
                ORDER BY t.created DESC")->queryAll();
        if ($bandera) {
    ...
    public function getReporteExctractarFormularios() {
        ...
        if ($limitQuery > 1) {
            ...
            for ($index = 1; $index <= $limitQuery; $index++) {
                ...
                $dataProviderLimit = array_merge($dataProviderLimit, Yii::$app->db->createCommand($sql)->queryAll());
            }
    ...
    public function extractConsTransORIGINAL() {
        ...
        //SI SOLO HABIA UNA VALORACIÃ“N PINTO LOS TITULOS
        if ($printTitle) {
            foreach ($titulos as $value) {
                $filecontent .= utf8_decode($value['header']) . "|";
            }
	...
	//controlar que no existen fechas null 
        ...
        if($this->arbol_id == '3104'){
            $titulos[19] = ['header' => 'Motivo contacto', 'value' => '19'];
            $titulos[20] = ['header' => 'Etapa del viaje', 'value' => '20'];
            $titulos[21] = ['header' => 'ID Encuesta', 'value' => '21'];
            $titulos[22] = ['header' => 'Email', 'value' => '22'];
            $titulos[23] = ['header' => 'Usuario COSV', 'value' => '23'];
            $titulos[24] = ['header' => 'Servicio Konecta', 'value' => '24'];
            $titulos[25] = ['header' => 'Nota anterior NPS', 'value' => '25'];
            $titulos[26] = ['header' => 'Valoracion Adicional y/o Escalada', 'value' => '26'];
            $titulos[27] = ['header' => 'Score', 'value' => '27'];        
            $titulos[28] = ['header' => $textos[0]['titulo'], 'value' => '28'];
            $titulos[29] = ['header' => $textos[1]['titulo'], 'value' => '29'];
            $titulos[30] = ['header' => $textos[2]['titulo'], 'value' => '30'];
            $titulos[31] = ['header' => $textos[3]['titulo'], 'value' => '31'];
            $titulos[32] = ['header' => $textos[4]['titulo'], 'value' => '32'];
            $titulos[33] = ['header' => $textos[5]['titulo'], 'value' => '33'];
            $titulos[34] = ['header' => $textos[6]['titulo'], 'value' => '34'];
            $titulos[35] = ['header' => $textos[7]['titulo'], 'value' => '35'];
            $titulos[36] = ['header' => $textos[8]['titulo'], 'value' => '36'];
            $titulos[37] = ['header' => $textos[9]['titulo'], 'value' => '37'];
        }
        else{
        ...
        if($this->arbol_id == '3104'){
            $sql .= ", ba.reason3 'reason3', ba.reason4 'reason4', ba.idnps 'idnps', ba.email 'email', ba.nmusuario 'nmusuario', ba.user_seg 'user_seg', ba.nota_anterior 'nota_anterior'";
        }
        ...
        if($this->arbol_id == '3104'){
            $sql .= " LEFT JOIN  tbl_base_Avon ba ON f.basesatisfaccion_id = ba.id";
        }
            ...
            if (count($data) > 0) {
                ...
                foreach ($data as $i => $row) {
                    ...
                    if ($row['fid'] != $fid) {
                        ...
                        $dataProvider[$newRow][0] = $this->vData($data[$i]['Fecha']);
                        $dataProvider[$newRow][1] = $this->vData($data[$i]['hora_inicial']);
                        $dataProvider[$newRow][2] = $this->vData($data[$i]['hora_final']);
                        
                        if ($this->vData($data[$i]['hora_inicial']) != "" AND $this->vData($data[$i]['hora_final']) != ""){
                            
                            //echo timespan(strtotime($this->vData($data[$i]['hora_inicial'])), strtotime($this->vData($data[$i]['hora_final']))); die;

                            //print_r("inicial: " . $this->vData($data[$i]['hora_inicial']) . " Final: " . $this->vData($data[$i]['hora_final'])); die;
                            //$final1 = $date->format('Y-m-d H:i:s');
                            //$inicial1 = $date->format('Y-m-d H:i:s');

                            //$final = $this->vData($data[$i]['hora_inicial']);
                            //$inicial = $this->vData($data[$i]['hora_final']);

                            $inicial = new DateTime($this->vData($data[$i]['hora_inicial']));
                            $final = new DateTime($this->vData($data[$i]['hora_final']));

                            // $inicial = DateTime::createFromFormat("Y-m-d H:i:s", $inicial);
                            // $final = DateTime::createFromFormat("Y-m-d H:i:s", $final);
                        ...
                        $dataProvider[$newRow][4] = $this->vData($data[$i]['cant_modificaciones']);
                        $dataProvider[$newRow][5] = $this->vData($data[$i]['tiempo_modificaciones']);

                        $dataProvider[$newRow][6] = $this->vData($data[$i]['Dimension']);
                        $dataProvider[$newRow][7] = $this->vData($data[$i]['ArbolPadre']);
                        $dataProvider[$newRow][8] = $this->vData($data[$i]['Arbol']);
                        $dataProvider[$newRow][9] = $this->vData($data[$i]['Formulario']);
                        $dataProvider[$newRow][10] = $this->vData($data[$i]['cedula_evaluado']);
                        $dataProvider[$newRow][11] = $this->vData($data[$i]['evaluado']);
                        $dataProvider[$newRow][12] = $this->vData($data[$i]['responsable']);
                        $dataProvider[$newRow][13] = $this->vData($data[$i]['evaluador']);
                        $dataProvider[$newRow][14] = $this->vData($data[$i]['rol']);
                        $dataProvider[$newRow][15] = $this->vData($data[$i]['fuente']);
                        $dataProvider[$newRow][16] = $this->vData($data[$i]['transacion']);
                        $dataProvider[$newRow][17] = $this->vData($data[$i]['equipo']);
                        $dataProvider[$newRow][18] = $this->vData($data[$i]['fdscomentario']);

                        if($this->arbol_id == '3104'){
                            
                            $dataProvider[$newRow][19] = $this->vData($data[$i]['reason3']);
                            $dataProvider[$newRow][20] = $this->vData($data[$i]['reason4']);
                            $dataProvider[$newRow][21] = $this->vData($data[$i]['idnps']);
                            $dataProvider[$newRow][22] = $this->vData($data[$i]['email']);
                            $dataProvider[$newRow][23] = $this->vData($data[$i]['nmusuario']);
                            $dataProvider[$newRow][24] = $this->vData($data[$i]['user_seg']);
                            $dataProvider[$newRow][25] = $this->vData($data[$i]['nota_anterior']);

                            if ($data[$i]['ejec_principal'] != '' && $data[$i]['estado'] != '') {
                                $dataProvider[$newRow][26] ='ValoraciÃ³n Escalada';
                            } else {
                                if ($data[$i]['ejec_principal'] != '') {
                                    $dataProvider[$newRow][26] ='ValoraciÃ³n Adicional, Id valoraciÃ³n principal:' . $data[$i]['ejec_principal'];
                                }else{
                                    $dataProvider[$newRow][26] = 'N/A';
                                }
                            }
                            $dataProvider[$newRow][27] = $this->vData($data[$i]['score']);
                            $dataProvider[$newRow][28] = $this->vData($data[$i]['fi1_nmcalculo']);
                            $dataProvider[$newRow][29] = $this->vData($data[$i]['fi2_nmcalculo']);
                            $dataProvider[$newRow][30] = $this->vData($data[$i]['fi3_nmcalculo']);
                            $dataProvider[$newRow][31] = $this->vData($data[$i]['fi4_nmcalculo']);
                            $dataProvider[$newRow][32] = $this->vData($data[$i]['fi5_nmcalculo']);
                            $dataProvider[$newRow][33] = $this->vData($data[$i]['fi6_nmcalculo']);
                            $dataProvider[$newRow][34] = $this->vData($data[$i]['fi7_nmcalculo']);
                            $dataProvider[$newRow][35] = $this->vData($data[$i]['fi8_nmcalculo']);
                            $dataProvider[$newRow][36] = $this->vData($data[$i]['fi9_nmcalculo']);
                            $dataProvider[$newRow][37] = $this->vData($data[$i]['fi10_nmcalculo']);
                        } else{

                            if ($data[$i]['ejec_principal'] != '' && $data[$i]['estado'] != '') {
                                $dataProvider[$newRow][19] ='ValoraciÃ³n Escalada';
                            } else {
                                if ($data[$i]['ejec_principal'] != '') {
                                    $dataProvider[$newRow][19] ='ValoraciÃ³n Adicional, Id valoraciÃ³n principal:' . $data[$i]['ejec_principal'];
                                }else{
                                    $dataProvider[$newRow][19] = 'N/A';
                                }
                            }
                            $dataProvider[$newRow][20] = $this->vData($data[$i]['score']);
                            $dataProvider[$newRow][21] = $this->vData($data[$i]['fi1_nmcalculo']);
                            $dataProvider[$newRow][22] = $this->vData($data[$i]['fi2_nmcalculo']);
                            $dataProvider[$newRow][23] = $this->vData($data[$i]['fi3_nmcalculo']);
                            $dataProvider[$newRow][24] = $this->vData($data[$i]['fi4_nmcalculo']);
                            $dataProvider[$newRow][25] = $this->vData($data[$i]['fi5_nmcalculo']);
                            $dataProvider[$newRow][26] = $this->vData($data[$i]['fi6_nmcalculo']);
                            $dataProvider[$newRow][27] = $this->vData($data[$i]['fi7_nmcalculo']);
                            $dataProvider[$newRow][28] = $this->vData($data[$i]['fi8_nmcalculo']);
                            $dataProvider[$newRow][29] = $this->vData($data[$i]['fi9_nmcalculo']);
                            $dataProvider[$newRow][30] = $this->vData($data[$i]['fi10_nmcalculo']);
                        }
    ...
    /**
     * Si viene algÃºn dato retorna el dato, de lo contrario retorne " - "
     * 
     * @return string
     */
    ...
    public static function getDatabygraf($arbol_id, $dimension_id, $fecha, $metrica, $banderaGrafica, $model, $control, $volumenes = false, $segundoCalifPer = false) {
        ...
        switch ($metrica) {
            case 13:
                ...
                break;
            case 14:
                $sql->andWhere("satu.tipologia = 'FELICITACI N CON BUZ N'");
                break;
            case 15:
                $sql->andWhere("satu.tipologia = 'CRITICA'");
                break;
            case 16:
                $sql->andWhere("satu.tipologia = 'CRITICA POR BUZ N'");
                break;
            case 17:
    ...
    public static function getDatabytabletotalexcel($dimension_id, $fecha, $metrica, $arbol, $model, $volumenes = false, $idMetrica = null) {
        ...
        switch ($metrica) {
            ...
            case 15:
                ...
                break;
            case 16:
                $sql->andWhere("AND satu.tipologia = 'CRITICA POR BUZ N'");
                break;
            case 17:
