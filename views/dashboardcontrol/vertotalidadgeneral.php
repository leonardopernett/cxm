...
<div class="CapaUno" style="display: inline" id="CapaUno">
    <table class="table table-striped table-bordered detail-view formDinamico" border="0">
        <thead>
            <tr>
                <th class="text-center"><?= Yii::t('app', 'Total Resultados') ?></th>
                <?php
                ...
                ?>
                    <th class="text-center"><?php echo 'Total '.$varMonth.' - '.$varYear; ?></th>
                <?php
                    }
                ?>
                <th class="text-center"><?= Yii::t('app', 'Total Promedio') ?></th>
            </tr>
        </thead>    
        <tbody>
            <tr>
            ...
            <?php
                ...
                foreach ($varMonthYear2 as $key => $value) {
                    ...
                    $varRtaTotal1 = Yii::$app->db->createCommand("select sum(cantidadvalor) as SumCantidad from tbl_control_volumenxcliente where mesyear = '$varMonth2' and anuladovxc = 0")->queryScalar();

                    $varRtaTotalidad = $varRtaTotal + $varRtaTotal1;

                    $varSumPromedio = $varSumPromedio + $varRtaTotalidad;
            ?>   
                <td class="text-center"><?php echo $varRtaTotalidad; ?></td>
            <?php
                }
            ?>
