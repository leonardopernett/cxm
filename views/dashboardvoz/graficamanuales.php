...
$this->title = 'Dashboard Ejecutivo (Programa VOC - Konecta)';
    ...
    if ($txtMesyear != null) {
        ...
        if ($txtServicio == '1') {
            $txtListDays = Yii::$app->db->createCommand("select distinct sum(tbl_control_volumenxclientedq.cantidadvalor) as cantidadvalor, tbl_control_volumenxclientedq.fechavaloracion from tbl_control_volumenxclientedq inner join tbl_arbols on tbl_control_volumenxclientedq.idservicio = tbl_arbols.id where tbl_arbols.arbol_id in (2,98) and tbl_control_volumenxclientedq.fechavaloracion between '$varFechaInicio' and '$varFechaFin' group by tbl_control_volumenxclientedq.fechavaloracion")->queryAll();

            $txtListDaysAuto = Yii::$app->db->createCommand("select distinct sum(tbl_control_volumenxclienteds.cantidadvalor) as cantidadvalor, tbl_control_volumenxclienteds.fechavaloracion AS fecha_sola from tbl_control_volumenxclienteds inner join tbl_arbols on tbl_control_volumenxclienteds.idservicio = tbl_arbols.id where tbl_arbols.arbol_id in (2,98) and tbl_control_volumenxclienteds.fechavaloracion between '$varFechaInicio' and '$varFechaFin' group by tbl_control_volumenxclienteds.fechavaloracion")->queryAll();
        }else{
            $txtListDays = Yii::$app->db->createCommand("select distinct sum(tbl_control_volumenxclientedq.cantidadvalor) as cantidadvalor, tbl_control_volumenxclientedq.fechavaloracion from tbl_control_volumenxclientedq inner join tbl_arbols on tbl_control_volumenxclientedq.idservicio = tbl_arbols.id where tbl_arbols.arbol_id in ($txtServicio) and tbl_control_volumenxclientedq.fechavaloracion between '$varFechaInicio' and '$varFechaFin' group by tbl_control_volumenxclientedq.fechavaloracion")->queryAll();

            $txtListDaysAuto = Yii::$app->db->createCommand("select distinct sum(tbl_control_volumenxclienteds.cantidadvalor) as cantidadvalor, tbl_control_volumenxclienteds.fechavaloracion AS fecha_sola from tbl_control_volumenxclienteds inner join tbl_arbols on tbl_control_volumenxclienteds.idservicio = tbl_arbols.id where tbl_arbols.arbol_id in ($txtServicio) and tbl_control_volumenxclienteds.fechavaloracion between '$varFechaInicio' and '$varFechaFin' group by tbl_control_volumenxclienteds.fechavaloracion")->queryAll();        }        
...
<div id="dtbloque1" class="col-sm-12" style="display: inline">
    <div id="idCapa0" style="display: inline">
        <div class="row">
            <div class="col-md-12">
                <div class="card1 mb">
                    <label><em class="fas fa-calendar-alt" style="font-size: 20px; color: #C148D0;"></em> Métricas de valoraciones en detalle por dia:</label>
