...
class EquiposEvaluados extends \yii\db\ActiveRecord {
    ...
    public static function getEvaluadosEquipo($idequipo = 0 ) {
        $sql = "SELECT REPLACE(ev.dsusuario_red, '�', 'n') as dsusuario_red, ee.evaluado_id, ev.identificacion as identificacion FROM tbl_equipos_evaluados ee INNER JOIN tbl_evaluados ev on ev.id = ee.evaluado_id WHERE ee.equipo_id = :idequipo";
