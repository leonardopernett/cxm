...
class ControlalinearvocController extends \yii\web\Controller
{
    ...
    public function actionCreatealinearvoc($arbol_idV, $tecnico_idV, $dimensionsV, $sesionV)
    {
        ...
        $txtNomPcrc = (new \yii\db\Query())
            ->select(['name'])
            ->from('tbl_arbols')
            ->where('id=:idarbol')
            ->addParams([':idarbol' => $txtPcrcS])
            ->scalar();


        $txtNomValora = (new \yii\db\Query())
            ->select(['name'])
            ->from('tbl_evaluados')
            ->where('id=:idvalorado')
            ->addParams([':idvalorado' => $txtValoradorS])
            ->scalar();
    ...
    public function actionCrearfeedback()
    {
        ...
        $txtRta = (new \yii\db\Query())
            ...
            ->addParams([':evaluadoid' => $txtEval])
            ->scalar();

        $txtLider = (new \yii\db\Query())
            ->select(['equipo_id'])
            ->from('tbl_equipos_evaluados')
            ->where('evaluado_id=:evaluadoid')
            ->addParams([':evaluadoid' => $txtRta])
            ->scalar();
    ...
    public function actionListashijo()
    {
        ...
        $txtRta = (new \yii\db\Query())
            ->select(['idlistahijovoc', 'nombrelistah'])
            ->from('tbl_controlvoc_listadohijo')
            ->where([
                'anulado' => 0,
                'idlistapadrevoc=:txttxtvmotivo'
            ])
            ->addParams([':txttxtvmotivo' => $txttxtvmotivo])
            ->all();
    ...
    public function actionCreatefocalizada()
    {
        ...
        ], "id_idvalorado = :txtvValorado and fechacreacion = :txtvFechacreacion and idbloque1 is null and anulado = 0")
        ...
        Yii::$app->db->createCommand()->update('tbl_medir_alinear', [
            'idbloque1' => $txtvIdBloque,
        ], "id_idvalorado = :txtvValorado and fechacreacion = :txtvFechacreacion and idbloque1 is null")
            ->bindValue(':txtvValorado', $txtvValorado)
            ->bindValue(':txtvFechacreacion', $txtvFechacreacion)
            ->execute();
    ...
    public function actionVerlistasalinearvoc($id)
    {
        ...
        $varValorador = (new \yii\db\Query())
            ->select(['valorador_id'])
            ->from('tbl_controlvoc_bloque1')
            ->where('idbloque1=:idbloque1')
            ->addParams([':idbloque1' => $txtIdVoc])
            ->scalar();



        $txtNomvalorador = (new \yii\db\Query())
            ->select(['usua_nombre'])
            ->from('tbl_usuarios')
            ->where('usua_id=:varValorador')
            ->addParams([':varValorador' => $varValorador])
            ->scalar();

        $varArbol = (new \yii\db\Query())
            ->select(['arbol_id'])
            ->from('tbl_controlvoc_bloque1')
            ->where('idbloque1=:idbloque1')
            ->addParams([':idbloque1' => $txtIdVoc])
        ...
        $txtArbol = (new \yii\db\Query())
            ->select(['name'])
            ->from('tbl_arbols')
            ->where('id=:varArbol')
            ->addParams([':varArbol' => $varArbol])
            ->scalar();

        $varTecnico = (new \yii\db\Query())
            ->select(['tecnico_id'])
            ->from('tbl_controlvoc_bloque1')
            ->where('idbloque1=:idbloque1')
            ->addParams([':idbloque1' => $txtIdVoc])
        ...
        $txtNombreTecnico = (new \yii\db\Query())
            ->select(['name'])
            ->from('tbl_evaluados')
            ->where('id=:varTecnico')
            ->addParams([':varTecnico' => $varTecnico])
            ->scalar();
        $txtDimensiones = (new \yii\db\Query())
            ->select(['dimensions'])
            ->from('tbl_controlvoc_bloque1')
            ->where('idbloque1=:idbloque1')
            ->addParams([':idbloque1' => $txtIdVoc])
        ...
        $txtFecha = (new \yii\db\Query())
            ->select(['fechahora'])
            ->from('tbl_controlvoc_bloque1')
            ->where('idbloque1=:idbloque1')
            ->addParams([':idbloque1' => $txtIdVoc])
            ->scalar();
        $txtAgente = (new \yii\db\Query())
            ->select(['usuagente'])
            ->from('tbl_controlvoc_bloque1')
            ->where('idbloque1=:idbloque1')
            ->addParams([':idbloque1' => $txtIdVoc])
        ...
        $txtPlanAccion = (new \yii\db\Query())
            ->select(['*'])
            ->from('tbl_control_alinear_bloque3')
            ->where('idbloque1=:idbloque1')
            ->addParams([':idbloque1' => $txtIdVoc])
            ->all();
        ...
        $txtCategorias = Yii::$app->db->createCommand("select tbl_arbols.name, tbl_categorias_alinear.categoria_nombre, tbl_atributos_alinear.atributo_nombre, tbl_atributos_alinear.medicion, tbl_atributos_alinear.acuerdo  from tbl_categorias_alinear
                ...
                WHERE tbl_categorias_alinear.arbol_id  = :varArbol")
        ...
        $varcodigos = Yii::$app->db->createCommand(" select tbl_control_alinear_participa.codigo_partcipa
                ...
                WHERE tbl_control_alinear_participa.idbloque1 = :txtIdVoc")
        ...
        for ($i = 0; $i < count($arrayCodigos); $i++) {
            ...
            $txtNombrePartic2 = Yii::$app->db->createCommand("select tbl_participantes.participan_nombre from tbl_participantes WHERE tbl_participantes.participan_id = ':valor'")
        ...
        if ($txtSesion2 == 3) {
            $txtContarSi1 = Yii::$app->db->createCommand("select COUNT(tbl_atributos_alinear.atributo_nombre) AS cuentasi
                    ...
                    where tbl_arbols.id = :arArbol and tbl_medir_alinear.id_idvalorado = :varTecnico and tbl_categorias_alinear.sesion_id in(1, 2) AND tbl_medir_alinear.medicion = 'Si' AND tbl_medir_alinear.idbloque1 = :txtIdVoc")
                ->bindValue(':arArbol', $varArbol)
            ...
            $txtContarNA1 = Yii::$app->db->createCommand("select COUNT(tbl_atributos_alinear.atributo_nombre) AS cuentasi
                    ...
                    INNER JOIN tbl_sesion_alinear on tbl_categorias_alinear.sesion_id = tbl_sesion_alinear.sesion_id
                    INNER JOIN tbl_medir_alinear ON tbl_medir_alinear.id_atrib_alin = tbl_atributos_alinear.id_atrib_alin
                    where tbl_arbols.id = :arArbol and tbl_medir_alinear.id_idvalorado = :varTecnico and tbl_categorias_alinear.sesion_id in(1, 2) AND tbl_medir_alinear.medicion = 'NA' AND tbl_medir_alinear.idbloque1 = :txtIdVoc")
                ->bindValue(':arArbol', $varArbol)
                ->bindValue(':varTecnico', $varTecnico)
                ->bindValue(':txtIdVoc', $txtIdVoc)
                ->queryScalar();
            ...
            $txtContarSesion1 = Yii::$app->db->createCommand("select COUNT(tbl_atributos_alinear.atributo_nombre) AS cuentasi
                   ...
                   WHERE tbl_categorias_alinear.arbol_id = :arArbol AND tbl_categorias_alinear.sesion_id in(1, 2) ")
                ->bindValue(':arArbol', $varArbol)
                ->queryScalar();
        } else {
            #code
        ...
        if ($txtSesion2 != 3) {
            $txtContarSi2 = Yii::$app->db->createCommand("select COUNT(tbl_atributos_alinear.atributo_nombre) AS cuentasi
                    ...
                    INNER JOIN tbl_sesion_alinear on tbl_categorias_alinear.sesion_id = tbl_sesion_alinear.sesion_id
                    INNER JOIN tbl_medir_alinear ON tbl_medir_alinear.id_atrib_alin = tbl_atributos_alinear.id_atrib_alin
                    where tbl_arbols.id = :arArbol and tbl_medir_alinear.id_idvalorado = :varTecnico and tbl_categorias_alinear.sesion_id = :txtSesion2 AND tbl_medir_alinear.medicion = 'Si' AND tbl_medir_alinear.idbloque1 = :txtIdVoc")
                ->bindValue(':arArbol', $varArbol)
                ->bindValue(':varTecnico', $varTecnico)
                ->bindValue(':txtSesion2', $txtSesion2)
                ->bindValue(':txtIdVoc', $txtIdVoc)
                ->queryScalar();
            ...
            $txtContarNA2 = Yii::$app->db->createCommand("select COUNT(tbl_atributos_alinear.atributo_nombre) AS cuentasi
                    ...
                    INNER JOIN tbl_sesion_alinear on tbl_categorias_alinear.sesion_id = tbl_sesion_alinear.sesion_id
                    INNER JOIN tbl_medir_alinear ON tbl_medir_alinear.id_atrib_alin = tbl_atributos_alinear.id_atrib_alin
                    where tbl_arbols.id = :arArbol and tbl_medir_alinear.id_idvalorado = :varTecnico and tbl_categorias_alinear.sesion_id = :txtSesion2 AND tbl_medir_alinear.medicion = 'NA' AND tbl_medir_alinear.idbloque1 = :txtIdVoc")
                ->bindValue(':arArbol', $varArbol)
                ->bindValue(':varTecnico', $varTecnico)
                ->bindValue(':txtSesion2', $txtSesion2)
                ->bindValue(':txtIdVoc', $txtIdVoc)
            ...
            $txtContarSesion2 = Yii::$app->db->createCommand("select COUNT(tbl_atributos_alinear.atributo_nombre) AS cuentasi
                    ...
                    INNER JOIN tbl_atributos_alinear ON tbl_atributos_alinear.id_categ_ali = tbl_categorias_alinear.id_categ_ali
                    INNER JOIN tbl_sesion_alinear ON tbl_sesion_alinear.sesion_id = tbl_categorias_alinear.sesion_id
                    WHERE tbl_categorias_alinear.arbol_id = :arArbol AND tbl_categorias_alinear.sesion_id =  :txtSesion2 ")
                ->bindValue(':arArbol', $varArbol)
                ->bindValue(':txtSesion2', $txtSesion2)
                ->queryScalar();
        } else {
    ...
    public function actionEditaralinearvoccat($var_idcat, $var_nombresesion)
    {
        ...
        $txtNombreList = (new \yii\db\Query())
            ->select(['categoria_nombre'])
            ->from('tbl_categorias_alinear')
            ->where(['id_categ_ali=:txtIdcat' and 'anulado' => 0])
            ->addParams([':txtIdcat' => $txtIdcat])
            ->Scalar();
    ...
    public function actionEditaralinearvocatri($var_idatri, $var_nombrecateg, $var_idcateg)
    {
        ...
        $txtNombreList = (new \yii\db\Query())
            ->select(['atributo_nombre'])
            ->from('tbl_atributos_alinear')
            ->where(['id_atrib_alin=:txtIdatri' and 'anulado' => 0])
            ->addParams([':txtIdatri' => $txtIdatri])
            ->Scalar();

        $txtidsesion = (new \yii\db\Query())
            ->select(['sesion_id'])
            ->from('tbl_categorias_alinear')
            ->where(['id_categ_ali=:txtIdcate' and 'anulado' => 0])
            ->addParams([':txtIdcate' => $txtIdcate])
            ->Scalar();

        $txtidsesion = (new \yii\db\Query())
            ->select(['sesion_nombre'])
            ->from('tbl_categorias_alinear')
            ->where(['sesion_id=:txtidsesion' and 'anulado' => 0])
            ->addParams([':txtidsesion' => $txtidsesion])
            ->Scalar();


        $txtsesionnombre = (new \yii\db\Query())
            ->select(['sesion_nombre'])
            ->from('tbl_categorias_alinear')
            ->where(['sesion_id=:txtidsesion' and 'anulado' => 0])
            ->addParams([':txtidsesion' => $txtidsesion])
