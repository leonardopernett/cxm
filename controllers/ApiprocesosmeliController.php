...
use app\models\Calificaciondetalles;
  ...
  class ApiprocesosmeliController extends \yii\web\Controller {
    ...
    public function behaviors(){
      return[
        ...
        'access' => [
            ...
            'rules' => [
              [
                'actions' => ['apivaloraciones','apiprocesosvalora','apiprocesosasesores','apiprocesosvaloradores','apiprocesosconexion'],
                'allow' => true,
                'roles' => ['@'],
    ...
    public function actionApivaloraciones(){
        ...
        var_dump(count($varListDataValoracion));

        die(json_encode("lOGRADO"));
    ...
    public function actionApiprocesosvaloradores(){
      ...
      $varListadoProcesosValoradores = (new \yii\db\Query())
                            ->select([
                              'tbl_conexionvaloracion_datosorigen.valorador_origen'
                            ])
                            ->from(['tbl_conexionvaloracion_datosorigen'])
                            ->where(['=','tbl_conexionvaloracion_datosorigen.anulado',0])
                            ->groupby(['tbl_conexionvaloracion_datosorigen.valorador_origen'])
                            ->all();


      foreach ($varListadoProcesosValoradores as $value) {
        $varOrigenValorador = $value['valorador_origen'];

        $paramsBuscaValorador = [':ProcesosValorador'=>$varOrigenValorador];
        $varValoradorJarvis = Yii::$app->dbjarvis->createCommand('
          SELECT dp_usuarios_actualizacion.documento FROM dp_usuarios_actualizacion
            WHERE 
              dp_usuarios_actualizacion.usuario = :ProcesosValorador')->bindValues($paramsBuscaValorador)->queryScalar();

        if ($varValoradorJarvis == "") {
          $varValoradorId = 0;
        }else{
          $varValoradorId = (new \yii\db\Query())
                                    ->select(['id'])
                                    ->from(['tbl_usuarios'])
                                    ->where(['=','tbl_usuarios.usua_identificacion',$varValoradorJarvis])
                                    ->scalar();
        }

        $varVerificarExistente = (new \yii\db\Query())
                                    ->select(['id_datosusuarios'])
                                    ->from(['tbl_conexionvaloracion_datosusuarios'])
                                    ->where(['=','tbl_conexionvaloracion_datosusuarios.usuariomeli_origen',$varOrigenValorador])
                                    ->count();

        if ($varVerificarExistente == 0) {
          Yii::$app->db->createCommand()->insert('tbl_conexionvaloracion_datosusuarios',[
                'documento_origen' => $varValoradorJarvis,
                'usuariokonecta_origen' => $varValoradorId,
                'usuariomeli_origen' => $varOrigenValorador,
                'activo' => 1,
                'fechacreacion' => date('Y-m-d'),
                'anulado' => 0,
                'usua_id' => 1,
    ...
    public function actionApiprocesosconexion(){
