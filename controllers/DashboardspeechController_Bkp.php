  ...
  class DashboardspeechController extends \yii\web\Controller {
    ...
    public function actionExport2(){
        ...
        if ($fechaIniCat < '2020-01-01') {
          ...
          if ($idArbol == '17' || $idArbol == '8' || $idArbol == '105' || $idArbol == '2575' || $idArbol == '1371' || $idArbol == '2253' || $idArbol == '675' || $idArbol == '3263' || $idArbol == '3070' ||  $idArbol == '3071' ||  $idArbol == '3077' || $idArbol == '3069' || $idArbol == '3110') {
...
// var_dump($txtIdIndicadores);
    ...
    public function actionListarpcrc(){            
      ...
      $txtanulado = 0;
        ...
        public function actionAutomaticspeech() {
          // $varClientes = Yii::$app->db->createCommand("select id_dp_clientes from tbl_speech_servicios where anulado = 0  and arbol_id != 1 group by id_dp_clientes")->queryAll();
          
          // foreach ($varClientes as $key => $value) {
          //   $varIdClientes = $value['id_dp_clientes'];

            $varPcrc = Yii::$app->db->createCommand("select cod_pcrc from tbl_speech_parametrizar where anulado = 0 and id_dp_clientes = 308 group by cod_pcrc")->queryAll(); 

            foreach ($varPcrc as $key => $value) {
              $varIdCodpcrc = $value['cod_pcrc'];

              $varRN = Yii::$app->db->createCommand("select rn from tbl_speech_parametrizar where anulado = 0 and cod_pcrc in ('$varIdCodpcrc') and rn != '' group by rn")->queryAll();
              $varExt = Yii::$app->db->createCommand("select ext from tbl_speech_parametrizar where anulado = 0 and cod_pcrc in ('$varIdCodpcrc') and ext != '' group by ext")->queryAll();
              $varUsu = Yii::$app->db->createCommand("select usuared from tbl_speech_parametrizar where anulado = 0 and cod_pcrc in ('$varIdCodpcrc') and usuared != '' group by usuared")->queryAll();

              $varListCategorias = Yii::$app->db->createCommand("select idcategoria from tbl_speech_categorias where anulado = 0 and cod_pcrc in ('$varIdCodpcrc') group by idcategoria")->queryAll();
              ...
              $varprograma = Yii::$app->db->createCommand("select programacategoria from tbl_speech_categorias where anulado = 0 and cod_pcrc in ('$varIdCodpcrc') group by programacategoria")->queryScalar();
              ...
              $varIdllamada = Yii::$app->db->createCommand("select idllamada from tbl_speech_servicios where anulado = 0 and id_dp_clientes = 308")->queryScalar();
              ...
              if (count($varRN) != 0) {
                ...
                if ($varIdllamada == "1105") {
                  $varListLlamadas = Yii::$app->get('dbSpeechE1')->createCommand("SELECT distinct (b.callId), a.categoryId as CAtegoriaID, a.name AS Nombre_Categoria, d.fieldValue as extension, d1.fieldValue AS login_id, DATEADD(s,c.callTime,'19700101') AS Fecha_Llamada, round(c.callduration,0) 'cantidadllamadas', e.name as Servicio  FROM [speechminer_8_5_512_E1].[dbo].[categoryInfoTbl] a, [speechminer_8_5_512_E1].[dbo].[callCategoryTbl] b, [speechminer_8_5_512_E1].[dbo].[callMetaTbl] c, [speechminer_8_5_512_E1].[dbo].[callMetaExTbl] d, [speechminer_8_5_512_E1].[dbo].[callMetaExTbl] d1, [speechminer_8_5_512_E1].[dbo].[programInfoTbl] e WHERE DATEADD(s,c.callTime,'19700101') BETWEEN '$varFechaDiaVencido' AND '$varFechaDiaActual' AND e.name = '$varprograma' AND a.categoryId in ($varIdllamada, $txtlistcategorias) AND d.fieldName='regla_negocio' AND d.fieldValue in ('$txtlistarn') AND d1.fieldName='login_id' AND a.categoryId = b.categoryId AND b.callId=c.callId AND d.callId=c.callId AND d1.callId=c.callId AND e.programId=c.programId ORDER BY Fecha_Llamada DESC")->queryAll();
                }else{
                  if ($varIdllamada == "1114") {
                    $varListLlamadas = Yii::$app->get('dbSpeechA2')->createCommand("SELECT distinct (b.callId), a.categoryId as CAtegoriaID, a.name AS Nombre_Categoria, d.fieldValue as extension, d1.fieldValue AS login_id, DATEADD(s,c.callTime,'19700101') AS Fecha_Llamada, round(c.callduration,0)  'cantidadllamadas', e.name as Servicio  FROM [speechminer_8_5_512_A2].[dbo].[categoryInfoTbl] a, [speechminer_8_5_512_A2].[dbo].[callCategoryTbl] b, [speechminer_8_5_512_A2].[dbo].[callMetaTbl] c, [speechminer_8_5_512_A2].[dbo].[callMetaExTbl] d, [speechminer_8_5_512_A2].[dbo].[callMetaExTbl] d1, [speechminer_8_5_512_A2].[dbo].[programInfoTbl] e WHERE DATEADD(s,c.callTime,'19700101') BETWEEN '$varFechaDiaVencido' AND '$varFechaDiaActual' AND e.name = '$varprograma' AND a.categoryId in ($varIdllamada, $txtlistcategorias) AND d.fieldName='regla_negocio' AND d.fieldValue in ('$txtlistarn') AND d1.fieldName='login_id' AND a.categoryId = b.categoryId AND b.callId=c.callId AND d.callId=c.callId AND d1.callId=c.callId AND e.programId=c.programId ORDER BY Fecha_Llamada DESC")->queryAll();
                  }
                }
