    ...
    public function behaviors() {
        return [
            ...
            'access' => [
                ...
                'denyCallback' => function ($rule, $action) {
                    ...
                    return $action->controller->redirect($url);
                        ...
                        'rules' => [
                            [
                                'actions' => ['index', 'dimensionlistmultiple', 'metricalistmultiple', 'metricalistmultipledetallada',
                                    'vistacorte'
                                    , 'indexcorte', 'update', 'volver', 'delete',
            ...
            public function actionIndex() {
                $modelBusqueda = \app\models\FiltrosControl::find()->where(['usua_id' => Yii::$app->user->identity->id,
                            'guardar_filtro' => 1])->one();
                ...
                $data->metrica = ArrayHelper::map(
                                ...
                                , 'id', 'detexto'); //limite metrica vista unica German Index
                //$data->metrica[] = 'Score';
                ...
                switch ($tipoCorte) {
                    ...
                    case 3:
                        ...
                        for ($i = $fechaInicio; $i <= $fechaFin; $i+=86400) {
                            $cortes[] = ['fechaI' => date('Y-m-d', $i) . ' 00:00:00',
                ...
                for ($index = 1; $index <= $meses; $index++) {
                    ...
                    if (count($cortesFecha) == 0) {
                        for ($i = 1; $i <= 5; $i++) {
                            ...
                            $cortesSemanaCalc [] = ['fechaI' => $inicioMes . ' 00:00:00',
                        ...
                        foreach ($cortesFecha as $corte) {
                            $cortesSemanaCalc[] = ['fechaI' => $corte->segmento_fecha_inicio,
                        ...
                        if (($cortesSemanaCalc[$i]['fechaI'] >= $fechas[0] || $cortesSemanaCalc[$i]['fechaF'] >=
                            ...
                            if (!isset($cortesSemanaCalc[$i + 1])) {
                                if ($fechas[1] <= $cortesSemanaCalc[$i]['fechaF']) {
                                    ...
                                    $cortesSemana[] = ['fechaI' => $cortesSemanaCalc[$i]['fechaI'],
                                        'fechaF' => $fechaFin];
                                }
                                if ($fechas[1] > $cortesSemanaCalc[$i]['fechaF']) {
                                    $cortesSemana[] = ['fechaI' => $cortesSemanaCalc[$i]['fechaI'],
                                    ...
                                    $cortesSemana[] = ['fechaI' => $fechaI . ' 00:00:00',
                                        'fechaF' => $fechas[1]];
                                }
                            } else {
            ...
            public function calcularCortesMes($rangofecha = null, $id_usuario = null, $tipo_corte = null) {
                ...
                do {
                    ...
                    if (count($cortesFecha) > 0) {
                        $arrayCorteTemp = ['fechaI' => $cortesFecha->segmento_fecha_inicio,
                        ...
                        if ($primeraIteraccion) {
                            if ($arrayCorteTemp['fechaI'] < $fechas[0]) {
                                ...
                                $inicioMes = date('Y-m-d', (strtotime('+1 day', strtotime($arrayCorteTemp['fechaF']))));
                                $cortesMesCalc[] = ['fechaI' => $arrayCorteTemp['fechaI'],
                            ...
                            if ($inicioMes < $arrayCorteTemp['fechaI']) {
                                ...
                                $cortesMesCalc[] = ['fechaI' => $inicioMes . ' 00:00:00',
                ...
                try {
                    ...
                    foreach ($filtrosEnvio as $value) {
                        ...
                        if ($value->guardar_filtro == 1) {
                            $metricas = \app\models\Metrica::find()->where("id IN(" . $value->ids_metricas . ")")->asArray()->all();
                            $metrica = $this->validarMetrica($metricas[0]['id']);
            ...
            private function getDataGraphAgrupDimen($model, $agrupar, $idsArboles) {
                ...
                if (count($consulta) > 0) {
                    ...
                    if ($consulta[0]['total'] != 0) {
                        ...
                        $datos['arbol2'][] = ['name' => 'Total', 'colorByPoint' => true,
                        ...
                        $datos['count2'][] = ['name' => 'Total', 'colorByPoint' => true,
                            'data' => $arrayTemp, 'dataLabels' => $arrayLabelsx];
                        ...
                        foreach ($consulta1 as $value) {
                            ...
                            $datos['dimension'][] = ['name' => $arbolPadre->name,
                                'data' => [(double) round($promedio, 2)]];
                        }
                        $arrayTemp = [];
                        foreach ($datos['dimension'] as $valueDimension) {
                            $arrayTemp[] = [$valueDimension['name'], $valueDimension['data'][0]];
                        }
                        $datos['dimension2'][] = ['name' => 'Total', 'colorByPoint' => true,
                            'data' => $arrayTemp, 'dataLabels' => $arrayLabelsx];
                        $data->showGraf = true;
            ...
            WHERE e.dimension_id IN($dimension) 
                ...
                " . $wherePersonas . "     
                " . $whereVolumen . "     
            GROUP BY DATE_FORMAT(e.created, '%Y%m%d'), e.arbol_id, e.dimension_id 
            ...
            ORDER BY e.dimension_id, e.created";
                    ...
                    if (count($resultData) > 0) {
                        ...
                        $datos['datosGrafiaSepaDimen_cant'][] = ['name' => 'Total', 'colorByPoint' => true,
                        ...
                        $datos['datosGrafiaSepaDimen_prom'][] = ['name' => 'Total', 'colorByPoint' => true,
