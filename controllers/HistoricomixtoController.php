...
use app\models\SpeechParametrizar;
  ...
  class HistoricomixtoController extends \yii\web\Controller {
    ...
    public function actionExportbase(){
      ...
      $phpExc->getActiveSheet()->SetCellValue('H6','Lider');
      ...
      $varListIndiVari = Yii::$app->db->createCommand("select idcategoria, nombre, idcategorias, tipoindicador, responsable from tbl_speech_categorias where anulado = 0 and idcategorias in (1,2) and programacategoria in ('$txtServicio') and cod_pcrc in ('$txtcodigoCC') group by idcategoria order by idcategorias asc")->queryAll();
  ...
  // fin Diego
      ...
      foreach ($varListIndiVari as $key => $value) {
        ...
        if($varListadorespo) {
          if($vartotalrespo == $numcol1){
            ...
            $phpExc->getActiveSheet()->setCellValue($lastColumn.$numCell, 'Resultado Automatico Pec');
            $phpExc->getActiveSheet()->getStyle($lastColumn.$numCell)->getFont()->setBold(true);
            $phpExc->getActiveSheet()->getStyle($lastColumn.$numCell)->applyFromArray($styleColor2);
            $lastColumn++;
    ...
    $varListIndiVari2 = Yii::$app->db->createCommand("select idcategoria, nombre, idcategorias, orientacionsmart, responsable from tbl_speech_categorias where anulado = 0 and idcategorias in (1,2) and programacategoria in ('$txtServicio') and cod_pcrc in ('$txtcodigoCC') and responsable = 1 group by idcategoria order by idcategorias asc")->queryAll();
...
// fin
      ...
      foreach ($varListMetadata as $key => $value) {
        ...
        $varProcesoPec = (new \yii\db\Query())
                ...
                ->where(['=','tbl_speech_general.callid',$txtCallid])
                ->andwhere(['=','tbl_speech_pecservicios.cod_pcrc',$txtcodigoCC])
                ->count(); 
        ...
        $phpExc->getActiveSheet()->setCellValue('F'.$numCell, $value['login_id']);
          ...
          $phpExc->getActiveSheet()->setCellValue('H'.$numCell, $varLider);
        ...
        foreach ($varListIndiVari as $key => $value) {
          ...
          if($varListadorespo) {
              ...
              if($cuentavari == ($vartotalrespo)) {
                  ...
                  $phpExc->getActiveSheet()->setCellValue($lastColumn.$numCell, $totalpondePec); 
                  $lastColumn++;
                  ...
                  $varScore = (new \yii\db\Query())
                                  ...
                                  ->where('tbl_speech_mixta.callId IN (:varCallids)',[':varCallids'=>$txtCallid])
                                  ->andwhere('tbl_speech_mixta.fechareal IN (:varFechareals)',[':varFechareals'=>$txtFecha])
                  ...
                  $phpExc->getActiveSheet()->setCellValue($lastColumn.$numCell, $varScoreValoracion); 
                  $lastColumn++;

                  $varPromedioScore = round(($totalpondeR + $varScoreValoracion) / 2,2);

                  $phpExc->getActiveSheet()->setCellValue($lastColumn.$numCell, $varPromedioScore); 
                  $lastColumn++;
              }
          }
